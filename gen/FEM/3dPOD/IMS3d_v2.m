...
cu1 = translate(cu1, [-xPos_FB-5e-3/2, -yPos_FB-5e-3/2, 0]);
gm = union(gm, cu1, "KeepBoundaries",true);

cu2 = fegeometry(multicuboid(l_FB+10e-3,b_FB+10e-3,h_Cu, 'ZOffset', z3));
cu2 = translate(cu2, [+xPos_FB+5e-3/2, -yPos_FB-5e-3/2, 0]);
...
gap2e = fegeometry(multicuboid(l_fix,l_fix,h_Ga, 'ZOffset', z0));
gap2e = translate(gap2e, [0, -b/2+l_fix, 0]);
...
thermalmodel.FaceLoad([107, 109]) = faceLoad(ConvectionCoefficient=hc, AmbientTemperature=Ta);
thermalmodel.FaceLoad([95, 97, 99, 101, 103, 105]) = faceLoad(ConvectionCoefficient=hfix, AmbientTemperature=Ta);
...
%% Solve Model
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
thermalmodel = generateMesh(thermalmodel,'Hmax',2*dx,'Hmin',dx,Hgrad=2);
...
function isAtCuPad = checkCuPadPosition(x, y)
    ...
    cuCenters = [
        -xPos_FB - 5e-3/2,  -yPos_FB - 5e-3/2;   % cu1
        +xPos_FB + 5e-3/2,  -yPos_FB - 5e-3/2    % cu2
    ];
