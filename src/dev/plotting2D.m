...
%% Description
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This functions plots the results for a single output node.
% Input:
%       - train:    input training data
%       - test:     input testing data
%       - val:      input validation data
%       - pred:     input predicted data
%       - mdl:      reduced order models
...
%% FNC
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = plotting2D(train, test, val, pred, mdl, mdl2, setup)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Init
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ----------------------------------------------------
...
% Para
% ----------------------------------------------------
selX = setup.plot.selX;                                                     % x-position for 1D temporal plots                           
selY = setup.plot.selY;                                                     % y-position for 1D temporal plots
selT = setup.plot.selT;                                                     % temporal instant for 1D/2D spatial plots (if -1 last sample)
Ts = setup.data.Ts;
dx = train.dx;
dy = train.dy;
...
subplot(1,3,1);
pcolor(x,y,squeeze(train.Mat2D.k));
...
subplot(1,3,2);
pcolor(x,y,squeeze(train.Mat2D.Cp));
...
% Mass Density
subplot(1,3,3);
pcolor(x,y,squeeze(train.Mat2D.rho));
...
% Training
subplot(2,3,1);
pcolor(x,y,squeeze(train.X{1}(end, :, :)));
...
colorbar
subplot(2,3,4);
pcolor(x,y,squeeze(train.y{1}(end, :, :)));
...
% Training
subplot(2,3,1);
plot(t_train, squeeze(train.X{1}(:, selX, selY)));
title("Heat Generation (Train)")
xlabel("t (sec)");
ylabel("q (W/m³)");
grid on
subplot(2,3,4);
plot(t_train, squeeze(train.y{1}(:, selX, selY)));
title("Temperatures (Train)")
xlabel("t (sec)");
ylabel("T (°C)");
grid on
